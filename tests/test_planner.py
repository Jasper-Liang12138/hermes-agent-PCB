...
# ====== 功能：验证 reroute 上下文完成后主 reroute 未配置时进入 help_planner。 ======
def test_reroute_after_context_uses_help_planner_fallback():
...
# ====== 功能：验证 fanout 路由完成后先等待导入确认。 ======
def test_fanout_route_result_waits_for_import_confirm():
    cache = {
        "projectData": "board.txt",
        "fanoutEntities": {"selectedBGA": "U5", "routerType": "rule_135"},
        "layerAssignResult": {"status": "ok"},
    ...
    assert plan["tool_calls"] == []
    assert plan["action"] == "route_review"
