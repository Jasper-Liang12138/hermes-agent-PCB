...
_WIDTH_RE = re.compile(
    r"(?:线宽|linewidth|line\s*width|width)\s*(?:改成|设为|设置为|为|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:mil)?",
    re.IGNORECASE,
)
_SPACING_RE = re.compile(
    r"(?:线距|spacing|line\s*spacing)\s*(?:改成|设为|设置为|为|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:mil)?",
    re.IGNORECASE,
)
_EXCLUDE_NET_TOKENS = {
    "arc",
...
def parse_fanout_target_from_text(text: Any) -> str:
    """Extract a targeted refdes/BGA from a single fanout request."""
    source = _as_text(text)
    if not source.strip():
        return ""
    patterns = (
        r"(?:对|给|帮我对|请对)\s*([A-Za-z]+\d+)\s*(?:进行)?\s*(?:PCB\s*)?(?:BGA\s*)?(?:fanout|逃逸布线|布线|重新生成参数)",
        r"(?:for|target)\s+([A-Za-z]+\d+)\s+(?:pcb\s+)?(?:bga\s+)?(?:fanout|routing|route)",
        r"([A-Za-z]+\d+)\s*(?:需要|进行|做)?\s*(?:PCB\s*)?(?:BGA\s*)?(?:fanout|逃逸布线|布线|重新生成参数)",
    )
