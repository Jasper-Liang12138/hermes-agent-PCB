"""PcbRouter ``-bga_local_route`` adapter for fanout fallback."""
...
def resolve_pcbrouter_binary() -> Path:
    ...
    default_dir = _repo_root() / "vendor" / "pcbrouter" / "bin"
    for name in _candidate_binary_names():
        candidate = default_dir / name
        if candidate.is_file():
            return candidate
    return default_dir / ("pcbrouter.exe" if sys.platform == "win32" else "pcbrouter")
    ...
    if header == b"\x7fELF":
        if not sys.platform.startswith("linux"):
            raise RuntimeError(
                ...
                "请在 Linux/天翼云环境执行，或配置 fake/script 版本做单元测试。"
            )
...
def _ordered_local_route_rows(fanout_params: dict[str, Any], project_data: str) -> list[tuple[str, str]]:
...
def _csv_override_path(fanout_params: dict[str, Any]) -> str:
    keys = ("pcbrouterCsvPath", "localRouteCsvPath", "bgaLocalRouteCsvPath", "csvPath")
    for key in keys:
        value = str(fanout_params.get(key) or "").strip()
        if value:
            return value
    return ""
...
def write_local_route_csv(
    *,
    ...
    override = _csv_override_path(fanout_params)
    target = work_dir / "bga_local_route_input.csv"
    if override:
    ...
    rows = _ordered_local_route_rows(fanout_params, project_data)
    if not rows:
        raise ValueError("pcbrouter fallback 缺少可写入 CSV 的目标 net")
...
def _prepare_board_input(
    *,
    ...
    if not str(project_data or "").strip():
        raise ValueError("pcbrouter fallback 缺少 KiCad PCB 输入内容")
    target = work_dir / "pcbrouter_input.kicad_pcb"
...
def _run_process(args: list[str], work_dir: Path, timeout: int) -> subprocess.CompletedProcess:
    logger.info("Executing pcbrouter fallback: %s in %s", args, work_dir)
...
def _resolve_routing_result_path(work_dir: Path, input_board_path: Path) -> Path:
    ...
    raise FileNotFoundError("pcbrouter 未生成可识别的 local route 结果文件")
...
def run_pcbrouter_local_route(
    *,
    ...
    """Run PcbRouter local BGA route mode in an isolated subdirectory."""
    if not isinstance(fanout_params, dict):
        raise ValueError("fanout_params 必须是 JSON 对象")

    run_dir = work_dir / "pcbrouter_local_route"
    ...
    input_csv_path = write_local_route_csv(
        ...
        project_data=input_board_path.read_text(encoding="utf-8", errors="replace"),
        work_dir=run_dir,
    )
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(f"pcbrouter fallback 执行失败 (exit {proc.returncode}):\n{output[:1600]}")
    ...
    report = (
        f"pcbrouter 局部布线兜底完成：目标 net {row_count} 个，"
