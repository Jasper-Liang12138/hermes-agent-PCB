"""Wrapper for the vendored PCB DRC agent report tool."""
...
def generate_drc_agent_report(
    *,
    ...
    """Run prod_main.py and return the Chinese agent-ready DRC JSON payload."""
    ...
    if not board_path.is_file():
        return {
            ...
            "error": f"DRC agent input board file is unavailable: {board_file_path}",
        }
    ...
    if not prod_main.is_file():
        return {
            "ok": False,
            "json_path": str(json_path),
            "error": f"DRC agent prod_main.py is missing: {prod_main}",
        }
    ...
    try:
        completed = subprocess.run(
            ...
            else float(os.getenv("PCB_DRC_AGENT_REPORT_TIMEOUT", str(DEFAULT_TIMEOUT_SECONDS))),
        )
    except Exception as exc:
        return {
            "ok": False,
            "json_path": str(json_path),
            "error": f"DRC agent report generation failed: {type(exc).__name__}: {exc}",
        }
    ...
    if not payload:
        return {
            ...
            "json_path": str(json_path),
            "returncode": completed.returncode,
            "error": "DRC agent completed but did not write JSON output.",
        }
