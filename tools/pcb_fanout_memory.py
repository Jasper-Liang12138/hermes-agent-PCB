"""Persistent fanout iteration memory for PCB BGA routing.

The in-process conversation is not the source of truth for iterative fanout
design.  This module stores compact, explicit run records and versioned
artifacts on disk so users can restore a prior round's layer assignment and
escape-order parameters after several routing attempts.
"""
...
_ENV_MEMORY_DIR = "PCB_FANOUT_MEMORY_DIR"
_SCHEMA_VERSION = 1
...
def _copy_file(source: Path | str, target: Path) -> bool:
...
def resolve_memory_root(configured_dir: Optional[str] = None) -> Path:
    raw = os.getenv(_ENV_MEMORY_DIR, "").strip() or str(configured_dir or "").strip()
    if raw:
        return Path(os.path.expandvars(os.path.expanduser(raw))).resolve()
    return (get_hermes_home() / "pcb_fanout_memory").resolve()
...
def parse_fanout_memory_request(text: Any) -> dict[str, Any]:
    """Parse a user request related to fanout run memory.

    Returns a dict with ``isMemoryRequest``.  Version values are concrete ints
    where possible; ``baseLayoutVersion`` may also be ``"current"``.
    """
    ...
    list_versions = bool(re.search(r"有哪些.*(?:版本|历史)|列出.*(?:版本|历史)|fanout.*历史|布线.*历史", source, re.IGNORECASE))
    if list_versions:
        return {"isMemoryRequest": True, "intent": "list_versions", "autoRoute": False}

    has_iteration_word = bool(re.search(r"再来一轮|再跑一轮|重新跑一轮|再试一轮|迭代|下一轮|重新生成", source, re.IGNORECASE))
    has_restore_word = bool(re.search(r"回到|恢复|还原|切回|退回|选用|采用|用|按|基于", source, re.IGNORECASE))
    ...
    if not (has_iteration_word or (has_restore_word and mentions_version) or (mentions_version and (mentions_param or mentions_layout))):
        return {"isMemoryRequest": False}
    ...
    if re.search(r"初始|原始", source):
        if mentions_layout or re.search(r"从.*(?:初始|原始)", source):
            base_layout = 0
        elif mentions_param:
            restored_from = 0
    ...
    if version is not None:
        before = source[: max(source.find(str(version)), 0)]
        after = source[source.find(str(version)) :] if str(version) in source else source
        window = after[:30]
        if mentions_layout and (re.search(r"从", before[-6:] + window) or re.search(r"版图|板图|layout|结果|routed", window, re.IGNORECASE)):
            base_layout = version
        if mentions_param or restored_from is None:
            restored_from = version

    if has_restore_word and mentions_param:
        intent = "rerun_from_params" if has_iteration_word or auto_route else "restore_params"
    elif has_iteration_word and restored_from is not None and mentions_param:
        intent = "rerun_from_params"
    elif has_iteration_word or base_layout is not None:
        intent = "iterate_from_layout"
    else:
        intent = "restore_params"

    if base_layout is None and re.search(r"从.*(?:初始|原始)", source):
        base_layout = 0
    if base_layout is None and re.search(r"从.*(?:当前|最新|上一轮|最后)", source):
        base_layout = "current"
...
class FanoutMemoryStore:
    """Disk-backed fanout memory store with a small session directory cache."""

    def __init__(self, configured_dir: Optional[str] = None):
        self._configured_dir = configured_dir
        self._lock = threading.RLock()
        self._session_dirs: dict[str, Path] = {}
        self._session_projects: dict[str, str] = {}

    def set_configured_dir(self, configured_dir: Optional[str]) -> None:
        with self._lock:
            if configured_dir:
                self._configured_dir = str(configured_dir)

    @property
    def root(self) -> Path:
        return resolve_memory_root(self._configured_dir)
    ...
    def bind_project(self, session_id: str, project_id: str) -> None:
        ...
        if project:
            with self._lock:
                self._session_projects[session_key] = project

    def _history_path(self, session_id: str) -> Optional[Path]:
        session_key = str(session_id or "").strip()
        if not session_key:
            return None
        path = self._session_dirs.get(session_key)
        if path:
    ...
    def _write_history(self, history_path: Path, history: dict[str, Any]) -> None:
    ...
    def fanout_params_for_version(self, session_id: str, version: Any) -> tuple[dict[str, Any], Optional[int]]:
        ...
        if not rel_path:
            return {}, version_int
        path = history_path.parent / str(rel_path)
    ...
    def latest_fanout_params(self, session_id: str) -> tuple[dict[str, Any], Optional[int]]:
        ...
        latest = history.get("latestVersion") if isinstance(history, dict) else None
        return self.fanout_params_for_version(session_id, latest)
    ...
    def record_route_version(
        ...
        *,
        ...
        copied_order = _copy_file(order_input_path, version_dir / "order_input.txt")
        copied_routed = _copy_file(routed_layout_path, version_dir / "routed_layout.txt")
        copied_import = _copy_file(import_lines_path, version_dir / "import_lines.out")
        ...
        nl_order_count = len(nl_order_lines) if isinstance(nl_order_lines, list) else 0
        ...
        record = {
            ...
            "naturalLanguageOrderLineCount": nl_order_count,
            "importStatus": "pending",
            "metrics": {},
            "files": {
                "fanoutParams": f"{version_name}/fanout_params.json",
                "report": f"{version_name}/report.txt",
            },
        }
        if copied_order:
            record["files"]["orderInput"] = f"{version_name}/order_input.txt"
        if copied_routed:
            record["files"]["routedLayout"] = f"{version_name}/routed_layout.txt"
        if copied_import:
            record["files"]["importLines"] = f"{version_name}/import_lines.out"
