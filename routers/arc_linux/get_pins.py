...
def extract_pins(component_block):
    ...
    for block in pin_blocks:
        ...
        # ⭐ 记录 H15/H37/H59
        if pin_name in ["H15", "H37", "H59"] and x_val != "":
            h_pin_x_map[pin_name] = x_val
    ...
    for pin in temp_pins:
        name = pin["PinName"]
        if name in TARGET_F_PINS:
            target_h = "H" + name[1:]
            if target_h in h_pin_x_map:
                pin["X"] = h_pin_x_map[target_h]

        # 转字符串
        pin["X"] = str(pin["X"]) if pin["X"] != "" else ""
        pin["Y"] = str(pin["Y"]) if pin["Y"] != "" else ""
        pins.append(pin)
...
def main():
    ...
    with open(OUTPUT_CSV, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(["PinNumber", "Padstack", "x", "y", "rotation"])
