...
def _run_main(work_dir: Path) -> int:
    return core.run_command(
        ["./135_main.exe", core.BASE_LAYOUT, core.BASE_ORDER],
        cwd=work_dir,
...
def evaluate_entries(
    ...
    core.write_order_entries(run_dir / core.BASE_ORDER, entries, footer_lines, layer_names)
    main_return_code = _run_main(run_dir)
    turn_return_code = 0
    if main_return_code == 0 and run_final_turn:
        turn_return_code = run_turn(run_dir)

    if main_return_code != 0 or turn_return_code != 0:
    ...
    if not keep_artifacts:
        shutil.rmtree(run_dir)
    return RouteCandidate(run_name, source, core.clone_entries(entries), stats, artifact_dir, missing_nets)


def prepare_baseline(output_root: Path | None = None) -> tuple[list[OrderEntry], list[str], list[str], RouteCandidate, core.BoardData]:
    board_data = core.load_board_data(core.PROJECT_DIR)
    entries = core.clone_entries(board_data.entries)
    if output_root is None:
        work_dir = core.PROJECT_DIR
        stats = core.compute_stats(work_dir)
        missing = core.read_missing_nets(work_dir)
        candidate = RouteCandidate("baseline", "baseline", entries, stats, work_dir, missing)
        return entries, board_data.footer_lines, board_data.layer_names, candidate, board_data

    candidate = evaluate_entries(
    ...
    if candidate is None:
        raise RuntimeError("135 exe baseline evaluation failed")
    return entries, board_data.footer_lines, board_data.layer_names, candidate, board_data
...
def write_candidate_artifacts(
    ...
    *,
    ...
    keep_files = [
        core.BASE_ORDER,
        core.BASE_ROUTE_OUTPUT,
        core.BASE_STATS_OUTPUT,
        core.BASE_LAYOUT,
        "main.log",
        "135_main.exe",
        "result.json",
    ...
    if candidate.run_dir is None:
        ...
        core.write_order_entries(output_dir / core.BASE_ORDER, candidate.entries, footer_lines, layer_names)
        main_return_code = _run_main(output_dir)
...
def entries_to_layer_array(entries: list[OrderEntry], layer_names: list[str]) -> np.ndarray:
    index = {layer: i for i, layer in enumerate(layer_names)}
    return np.asarray([index[p.layer] for p in entries], dtype=np.int64)
