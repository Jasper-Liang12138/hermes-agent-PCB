...
def _branch_reaches_outside_bbox(board, pad, seed_point, bbox, max_depth=200,log_fn=None):
    """
    Check if the branch starting from seed_point can continuously reach
    any graph node outside the BGA bbox.
    """
    graph, _ = _build_net_connectivity_graph(board, pad.net)
    ...
    while queue:
        ...
        if _node_hits_valid_non_bga_pad(board, pad, cur):
            if log_fn:
                log_fn("debug", f"[H3 BFS] HIT valid non-BGA pad at cur={cur} depth={depth}")
            return True
...
def _node_hits_valid_non_bga_pad(board, src_pad, node, tol=0.12):
    ...
    for pad in board.pads:
        ...
        # 排除当前 BGA 自己内部的 pad
        if getattr(pad, "component", None) == getattr(src_pad, "component", None):
            continue

        if near_point(node, (pad.x, pad.y), tol):
            return True
