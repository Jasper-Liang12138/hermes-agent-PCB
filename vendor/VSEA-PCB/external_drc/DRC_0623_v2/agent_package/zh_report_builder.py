...
RULE_ZH = {
    ...
    "HR_DRC_PAD_ARC_CROSSING": {
        "name": "BGA区域内异网焊盘与圆弧走线短路",
        "explanation": "目标BGA区域内，不同网络的焊盘与同层圆弧铜走线发生实体重叠。",
        "impact": "可能造成不同网络短路，属于Hard错误。",
        "suggestion": "调整圆弧路径、所在层或焊盘附近的扇出方式。",
    },
    "HR_DRC_PAD_VIA_CROSSING": {
        "name": "BGA区域内异网焊盘与过孔短路",
        "explanation": "目标BGA焊盘与其他网络过孔的铜环在共同铜层发生实体重叠。",
        "impact": "可能造成焊盘网络与过孔网络短路，属于Hard错误。",
        "suggestion": "移动过孔或调整焊盘附近的扇出位置。",
    },
    "HR_DRC_VIA_TRACK_CROSSING": {
        "name": "BGA区域内异网过孔与走线短路",
        "explanation": "目标BGA区域内的过孔与其他网络的直线或圆弧铜走线发生实体重叠。",
        "impact": "可能造成过孔网络与走线网络短路，属于Hard错误。",
        "suggestion": "移动过孔或重布异网走线，消除铜实体重叠。",
    },
    ...
    "HR_CONNECT_VIA_ONLY_ESCAPE": {
        "name": "BGA焊盘仅连接到区域内过孔",
        "explanation": "该BGA信号焊盘已连接到过孔，但路径尚未走出BGA区域，也未连接到其他焊盘。",
        "impact": "当前连接不作为硬错误，但逃逸路径仍未完整完成。",
        "suggestion": "继续从该过孔布线到BGA区域外，或连接到目标焊盘。",
    },
...
def build_message_zh(result: Dict[str, Any], issues_zh: List[Dict[str, Any]]) -> str:
    ...
    hard_count = sum(
        1 for issue in result.get("hard_issues", [])
    ...
    warning_issues = [
        issue for issue in issues_zh
        if (issue.get("severity") or "").upper() == "WARNING"
    ]
    if hard_count == 0 and warning_issues:
        lines.append(f"当前没有 Hard 错误，另有 {len(warning_issues)} 个警告提醒：")
        for idx, issue in enumerate(warning_issues, start=1):
            lines.append(
