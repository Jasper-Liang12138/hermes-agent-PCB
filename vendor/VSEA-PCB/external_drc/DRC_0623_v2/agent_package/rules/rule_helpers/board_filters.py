...
def _is_no_connect_net(net: str) -> bool:
    if not _is_named_net(net):
        return False
    normalized = net.strip().lstrip("/").upper()
    return re.fullmatch(r"NC\d*", normalized) is not None


def _is_nc_pad(pad) -> bool:
    pad_id = str(getattr(pad, "id", "") or "")
    pad_name = pad_id.rsplit(".", 1)[-1].strip().upper()
    return (
        re.fullmatch(r"NC\d*", pad_name) is not None
        or _is_no_connect_net(getattr(pad, "net", ""))
    )
...
def _is_signal_net(net: str) -> bool:
    ...
    return (
        _is_named_net(net)
        and not _is_no_connect_net(net)
        and not _is_power_or_ground_net(net)
    )
